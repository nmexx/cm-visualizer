...
/** Maximum rows returned by computeFoilPremium. */
const MAX_FOIL_ROWS = 50;
...
function computeInventory(boughtItems, soldItems) {
  ...
  for (const item of boughtItems) {
    ...
    b.qty  += (item.quantity || 1);
    b.cost += (item.price || 0) * (item.quantity || 1);
    // Latch the first valid product_id seen for this card+set key
    if (!b.product_id && item.product_id) { b.product_id = item.product_id; }
  }
...
function computeRepeatBuyers(orders) {
  ...
  const dist = { once: 0, twice: 0, thrice: 0, more: 0 };
  for (const b of buyers) {
    if      (b.order_count === 1) { dist.once++; }
    else if (b.order_count === 2) { dist.twice++; }
    else if (b.order_count === 3) { dist.thrice++; }
    else                          { dist.more++; }
  }
...
function computeSetROI(soldItems, boughtItems) {
  ...
  // Include sets that were sold but never purchased (no cost basis → roi_pct null)
  for (const [set_name, s] of sell) {
    if (buy.has(set_name)) { continue; }
    const avgSell = s.qty > 0 ? s.revenue / s.qty : 0;
    results.push({
      set_name, qty_bought: 0, qty_sold: s.qty,
      total_cost: 0, total_revenue: s.revenue,
      avg_buy: 0, avg_sell: avgSell, roi_pct: null,
    });
  }
  results.sort((a, b) => {
...
function computeFoilPremium(soldItems) {
  ...
  return results.slice(0, MAX_FOIL_ROWS);
}
