...
const MIGRATIONS = [
  ...
  },
  {
    ...
    up(db) {
      // Check if the manabox_inventory table exists (it won't in fresh installs that use baseline DDL)
      const tableExists = db.prepare(`
        SELECT name FROM sqlite_master WHERE type='table' AND name='manabox_inventory'
      `).get();
      
      if (!tableExists) { return; } // Fresh install — baseline DDL has constraint already
  ...
  {
    id: 5,
    description: 'Add exclude_from_pl column to purchase_items and order_items',
    up(db) {
      // Only add columns to tables that exist (fresh installs get them from baseline DDL)
      
      // Check if purchase_items table exists and needs the column
      const purchaseItemRows = db.prepare(`
        SELECT name FROM sqlite_master WHERE type='table' AND name='purchase_items'
      `).all();
      if (purchaseItemRows.length > 0) {
        const cols = db.prepare(`PRAGMA table_info(purchase_items)`).all().map(c => c.name);
        if (!cols.includes('exclude_from_pl')) {
          db.exec(`ALTER TABLE purchase_items ADD COLUMN exclude_from_pl INTEGER DEFAULT 0;`);
        }
      }

      // Check if order_items table exists and needs the column  
      ...
      if (orderItemRows.length > 0) {
        ...
        if (!cols.includes('exclude_from_pl')) {
          db.exec(`ALTER TABLE order_items ADD COLUMN exclude_from_pl INTEGER DEFAULT 0;`);
        }
      }
    },
  },
