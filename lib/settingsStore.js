...
class SettingsStore {
  ...
  constructor(db) {
    ...
    this._presetsStmt = db.prepare("SELECT key, value FROM settings WHERE key LIKE 'preset_%'");
  }
  ...
  /**
   * Return all filter presets as an array: [{ name, from, to }, ...]
   * @returns {{ name: string, from: string, to: string }[]}
   */
  getPresets() {
    return this._presetsStmt.all().flatMap(r => {
      try {
        return [{ name: r.key.slice(7), ...JSON.parse(r.value) }];
      } catch {
        console.warn(`[settingsStore] preset '${r.key}' has invalid JSON — skipping`);
        return [];
      }
    });
  }
